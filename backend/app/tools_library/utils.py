# utils.py
"""
Shared utility functions for the tools library.
"""
import os
import json
import difflib
...
def core_ocr_extract(image_path: str, light: bool = False, group_by: str = 'word', search_text: str = None) -> str:
    """
    Reusable Core Logic for OCR extraction.
    Can be imported by other tools (like PDF tools) to process images.
    
    Args:
        image_path (str): Path to the image file.
        light (bool): Minimal output if True.
        group_by (str): 'word', 'line', 'block'.
        ...
        if group_by == 'word':
            ...
            for item in items:
                ...
                if group_by == 'line':
                    ...
                    # Unique key for a block/paragraph: Page, Block
                    key = (item.get('page_num'), item.get('block_num'))
                else:
                    # Fallback to word
                    key = i 
                ...
                # Filter meaningless text for aggregation
                txt = item['text']
