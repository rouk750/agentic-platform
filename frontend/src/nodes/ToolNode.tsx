    ...
    >
      ...
      <div className="flex items-center gap-3 p-3 border-b border-slate-100 bg-gradient-to-b from-white to-slate-50/50 rounded-t-xl">
        ...
        <div className="flex-1 min-w-0">
          ...
          />
          <div className="text-[10px] text-slate-400 font-medium">Executes active tools</div>
        </div>
        ...
        {hasStats ? (
          <div className="mt-2 space-y-1 border-t border-slate-100 pt-2">
            {Object.entries(toolStats).map(([toolName, count]) => (
              ...
              >
                <span className="truncate max-w-[140px]" title={toolName}>
                  {toolName}
                </span>
                <span className="bg-slate-100 px-1.5 py-0.5 rounded-full text-slate-600 font-semibold">
                  {count}
                </span>
              </div>
            ))}
