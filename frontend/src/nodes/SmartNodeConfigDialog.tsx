...
export function SmartNodeConfigDialog({ open, onOpenChange, data, onUpdate }: SmartNodeConfigDialogProps) {
    ...
    const handleUpdateExample = (index: number, type: 'inputs' | 'outputs', key: string, value: string) => {
    ...
    const parseCSV = (content: string) => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (hasData) {
                ...
                inputKeys.forEach((k: any) => { if (!example.inputs[k]) example.inputs[k] = ""; });
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl bg-white rounded-xl shadow-2xl z-50 p-0 border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
                        {activeTab === 'config' ? (
                            <form id="config-form" onSubmit={handleSubmit((data) => onSubmit(data, true))} className="flex flex-col gap-6 overflow-y-auto pr-2 min-h-0">
                                {/* General Settings */}
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        >
                                            ...
                                            <p className="text-xs text-slate-500">Best for reasoning, explanation, and complex tasks. Generates rationale before answer.</p>
                                        </div>
                                        ...
                                        >
                                            ...
                                            <p className="text-xs text-slate-500">Best for simple extraction, classification, or strict output formats. Cheaper & faster.</p>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            {inputFields.map((field, index) => (
                                                <div key={field.id} className="flex gap-2 items-start">
                                                    <div className="grid gap-1 flex-1">
                                                        <input {...register(`inputs.${index}.name`)} placeholder="Field name (e.g. question)" className="w-full text-xs p-1.5 rounded border border-slate-300 font-mono focus:border-blue-500 outline-none" />
                                                        <input {...register(`inputs.${index}.desc`)} placeholder="Description" className="w-full text-xs p-1.5 rounded border border-slate-300 focus:border-blue-500 outline-none" />
                                                    </div>
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center justify-between">
                                    <p className="text-sm text-slate-600">
                                        Providing examples helps DSPy learn the best way to prompt the LLM for your specific task.<br />
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex items-center gap-2 border-r border-slate-200 pr-3 mr-1">
                                            <label className="text-xs font-semibold text-slate-500 whitespace-nowrap">Rounds:</label>
                                            <input
                                                type="number"
                                                min="1"
                                                max="50"
                                                value={maxRounds}
                                                onChange={(e) => setMaxRounds(Math.max(1, Math.min(50, parseInt(e.target.value) || 1)))}
                                ...
                                {optimizationResult && (
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-800 flex items-center justify-between">
                                        <span className="flex items-center gap-2"><Sparkles size={14} /> Node Optimized Successfully!</span>
                                                    ...
                                                    <td className="p-3 border-r border-slate-200 align-top">
                                                        <div className="space-y-2">
                                                            {Object.entries(ex.inputs).map(([key, val]) => (
                                                                <div key={key}>
                                                                    <label className="text-[10px] text-blue-400 uppercase font-mono block mb-0.5">{key}</label>
                                                                    <textarea
                                                                        value={val as string}
                                                                        onChange={(e) => handleUpdateExample(index, 'inputs', key, e.target.value)}
                                                    ...
                                                    <td className="p-3 align-top">
                                                        <div className="space-y-2">
                                                            {Object.entries(ex.outputs).map(([key, val]) => (
                                                                <div key={key}>
                                                                    <label className="text-[10px] text-green-400 uppercase font-mono block mb-0.5">{key}</label>
                                                                    <textarea
                                                                        value={val as string}
                                                                        onChange={(e) => handleUpdateExample(index, 'outputs', key, e.target.value)}
                            ...
                            <>
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        const formData = watch();
                                        const selectedModel = models.find(m => m.id?.toString() === formData.llm_profile);
                                        onUpdate({ ...formData, llm_profile: selectedModel, examples, maxRounds, optimizationResult });
            ...
            <Dialog.Root open={showPasteDialog} onOpenChange={setShowPasteDialog}>
                <Dialog.Portal>
                    ...
                    <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-xl shadow-xl border border-slate-200 p-6 z-[61] outline-none">
                        <Dialog.Title className="text-lg font-semibold text-slate-800 mb-2">Import CSV</Dialog.Title>
                        <Dialog.Description className="text-sm text-slate-500 mb-4">
                            Paste your CSV content below. The first row must contain headers matching your Input/Output names.
                        </Dialog.Description>
