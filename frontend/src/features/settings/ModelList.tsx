...
export function ModelList({ models, onDelete, onEdit, onTest }: ModelListProps) {
    ...
    return (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {models.map((model) => (
                <div key={model.id} className="group relative bg-slate-100 border border-slate-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            {testingId === model.id ? <Loader2 size={16} className="animate-spin" /> : <Wifi size={16} />}
                        </button>
                        <button
