...
export const useRunStore = create<RunState>((set) => ({
  ...
  addToolExecution: (nodeId, toolName) =>
    set((state) => {
  ...
  updateIteratorProgress: (nodeId, current, total) =>
    set((state) => ({
      // Implemented
      iteratorProgress: {
  ...
  appendToken: (token, nodeId) =>
    set((state) => {
      ...
      const effectiveName = effectiveNodeId
        ? state.nodeLabels[effectiveNodeId] || effectiveNodeId
        : undefined;
      ...
      return {
        ...
        logs: [
          ...state.logs,
          {
            ...
            level: 'info',
            details: { nodeId, data: enrichedSnapshot },
            timestamp: Date.now(),
          },
        ],
      };
  ...
  completeNodeMessage: (nodeId) =>
    set((state) => {
      const messages = [...state.messages];
      for (let i = messages.length - 1; i >= 0; i--) {
        if (messages[i].role === 'user') break;

        if (messages[i].nodeId === nodeId && messages[i].role === 'ai') {
          messages[i] = { ...messages[i], isComplete: true };
          break;
        }
      }
      return { messages };
