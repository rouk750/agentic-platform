...
export function useVersionHistory<TVersion extends { id: number }, TRestored = unknown>(
  options: VersionHistoryOptions<TVersion, TRestored>
) {
  ...
  const handleBulkDelete = async (id: number) => {
    ...
    if (!confirm(`Are you sure you want to delete these ${selectedVersionIds.size} versions?`))
      return;
  ...
  const handleToggleLock = async (id: number, version: TVersion & { is_locked?: boolean }) => {
    ...
    try {
      ...
      // Update local state
      setVersions(
        versions.map((v) => (v.id === version.id ? { ...v, is_locked: newLockState } : v))
      );
    } catch (error) {
